...
if __name__=='__main__':
    ...
    all_features=feature_columns[8:]+list(str(i) for i in range(-24, 0)) + \
                   list([(attr + '_48ago') for attr in ['t2m_obs', 'rh2m_obs', 'w10m_obs', 'psur_obs', 'q2m_obs', 'u10m_obs',
                                                   'v10m_obs', 'RAIN_obs']])
    ...
    if opt.boost_type=="lgb":
        model_dir='../checkpoints/lgb_global'
    elif opt.boost_type=="catboost":
        model_dir = '../checkpoints/catboost_global'
    else:
    ...
    for feature_index in range(3):
        ...
        for i in range(90001,90011):
            ...
            if train_Xs is None:
                ...
                train_Xs=np.r_[train_Xs,X_train]

            if train_ys is None:
                train_ys=y_train
            else:
                train_ys=np.r_[train_ys,y_train]

            if train_ys_M is None:
                train_ys_M=y_train_M
            else:
                train_ys_M=np.r_[train_ys_M,y_train_M]

            if val_Xs is None:
                val_Xs = X_test
            else:
                val_Xs = np.r_[val_Xs, X_test]

            if val_ys is None:
                val_ys = y_test
            else:
                val_ys = np.r_[val_ys, y_test]

            if val_ys_M is None:
                val_ys_M = y_test_M
            else:
                val_ys_M = np.r_[val_ys_M, y_test_M]
        ...
        joblib.dump(model,
                    os.path.join(model_dir, 'model_feature_' + str(feature_index) + '.m'))

        print(model.feature_importances_)

        y_train_pred=model.predict(train_Xs)
        y_test_pred = model.predict(val_Xs)

        rmse_M = np.sqrt(mean_squared_error(val_ys, val_ys_M))
        rmse_FORE = np.sqrt(mean_squared_error(val_ys, y_test_pred))

        train_rmse_M=np.sqrt(mean_squared_error(train_ys,train_ys_M))
        train_rmse_FORE=np.sqrt(mean_squared_error(train_ys,y_train_pred))
        ...
        for ii, feature in enumerate(all_features):
            feature_importance_dict[target_list[feature_index]][feature]=(
                float(model.feature_importances_[ii]))
        feature_importance_dict['train_score'][target_list[feature_index]]=train_score
        feature_importance_dict['val_score'][target_list[feature_index]]=val_score
    ...
    feature_importance_dict['train_score']['mean'] = (feature_importance_dict['train_score']['t2m'] +
                                                      feature_importance_dict['train_score']['rh2m'] +
                                                      feature_importance_dict['train_score']['w10m'] ) / 3
    feature_importance_dict['val_score']['mean'] = (feature_importance_dict['val_score']['t2m']+
                                                    feature_importance_dict['val_score']['rh2m'] +
                                                    feature_importance_dict['val_score']['w10m']) / 3
